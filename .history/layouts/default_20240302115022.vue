<template class="w-scree h-screen">
    ...
    <div id="side-container" class="fixed inset-0 h-full w-full invisible">
        <div id="over-bg" class="absolute h-full w-full inset-0 bg-gray-900 opacity-0"></div>
        <div id="side-menu" class="absolute w-96 h-full bg-green-500 right-0 top-0 translate-x-full">
    ...
    <div class="w-full min-h-screen flex flex-col bg-sky-50 select-none">
        <header class="shadow-md shadow-sky-50 container mx-auto">
            <div class="flex justify-between items-center">
                ...
                <div>
                    <Icon @click="showMenu" name="material-symbols:menu-open-rounded" class="text-4xl cursor-pointer text-sky-900" />
                </div>
            </div>
...
<script>
    function showMenu() {
        ...
        document.querySelector('#over-bg').classList.toggle('invisible')
        document.querySelector('#side-menu').classList.toggle('invisible')
    }
</script>
