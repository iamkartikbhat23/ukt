<template class="w-scree h-screen">
    ...
    <div id="side-container" class="z-20 fixed inset-0 h-full w-full invisible">
        <div @click="toggleMenu(false)" id="over-bg" class="absolute duration-500 ease-out transition-all h-full w-full inset-0 bg-gray-900 opacity-0"></div>
        <div id="side-menu" class="absolute duration-500 ease-out transition-all w-[85%] md:w-96 h-full bg-white/90 right-0 top-0 translate-x-full">
            <div class="size-10 flex w-full p-2 justify-end cursor-pointer">
                <Icon @click="toggleMenu(false)" name="material-symbols:close" class="text-3xl cursor-pointer text-sky-900" />
            </div>
            <div class="py-4 md:py-10 flex flex-col items-center gap-2 p-2">
                ...
                <NuxtLink to="/" class="w-full border border-sky-800 px-4 py-3  rounded bg-gray-200">
                    <div>Dashboard</div>
                </NuxtLink>
                <NuxtLink to="/news/category" class="w-full border border-sky-800 px-4 py-3  rounded bg-gray-200">
                    <div >Categories</div>
                </NuxtLink>
    ...
    <div class="w-full min-h-screen flex flex-col bg-sky-50 select-none">
        <header class="shadow-md shadow-sky-50 container mx-auto">
            <div class="flex justify-between items-center">
                ...
                <div>
                    <Icon @click="toggleMenu(true)" name="material-symbols:menu-open-rounded" class="text-4xl cursor-pointer text-sky-900" />
                </div>
            </div>
...
<script setup>
    ...
    onMounted(() => {
        alert(sideMenuOpen.value)
        toggleMenu(sideMenuOpen.value) ;
    })
    ...
    function toggleMenu(status) {
        if(status) {
            document.querySelector('#side-container').classList.remove('invisible')
            document.querySelector('#over-bg').classList.remove('opacity-0')
            document.querySelector('#over-bg').classList.add('opacity-50')
            document.querySelector('#side-menu').classList.remove('translate-x-full')
        } else {
            document.querySelector('#side-container').classList.add('invisible')
            document.querySelector('#over-bg').classList.add('opacity-0')
            document.querySelector('#over-bg').classList.remove('opacity-50')
