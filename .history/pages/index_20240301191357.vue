...
<template>
    <div class="w-full">
                ...
                <SwiperSlide v-for="info in data?.news" class="px-1 md:px-0">
                    <div class="flex flex-col border border-blue-100 rounded-lg ">
                        <NuxtLink :to="`/news/${info.slug}`">
                            <img :src="info.main_image" alt="News card's main image" class="rounded-md h-fit w-full hover:rotate-[0.5deg] hover:shadow-md shadow-inner shadow-indigo-500/40"/>
                            <div class="p-1">
        ...
        <div class="my-2 md:py-10 flex justify-center gap-5">
            <div class=" cursor-pointer max-w-max px-3 py-1 border border-purple-500 text-sky-600 rounded-full hover:bg-purple-500 hover:text-white text-xl" v-for="categoryInfo in data?.popularCategories">
