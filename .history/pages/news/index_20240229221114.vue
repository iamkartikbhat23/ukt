<script setup>
    ...
    const { data, refresh  } = await useFetch(() => `/api/news?offset=${page}`) ;
    ...
    const perviousPg = async () => {
      const { data  } = await useFetch(`/api/news?offset=${--page.value}`) ;
      refresh() ;
    };
</script>
<template >
    <div class="flex flex-col p-1 md:p-0">
        ...
        <div class="flex w-full px-3 text-xl py-5" :class="page>1?'justify-between':'justify-end'">
            <div @click="nextPg" v-if="page>1" class="flex gap-1 items-center text-blue-500">
                <Icon name="mdi:chevron-double-left" color="#0ea5e9" class="text-xl"/>
                <div class="hidden md:block">Previous</div>
            </div>
            <div @click="perviousPg" v-if="data?.length==96" class="flex gap-1 items-center text-blue-500">
