...
<template >
    <div class="flex flex-col p-1 md:p-0">
        ...
        <div class="flex w-full px-3 text-xl py-5" :class="page>1?'justify-between':'justify-end'">
            <div @click="perviousPg()"  class="w-1/3 cursor-pointer flex gap-1 items-center text-blue-500">
                <div  v-if="page>1">
                    <Icon name="mdi:chevron-double-left" color="#0ea5e9" class="text-xl"/>
                    <div class="hidden md:block">Previous</div>
                </div>
            </div>
            <div class="w-1/3 text-xl">Page : {{ page+1 }}</div>
            <div @click="nextPg()"  class="w-1/3 cursor-pointer flex gap-1 items-center text-blue-500">
                <div v-if="data?.length==96" class="flex">
