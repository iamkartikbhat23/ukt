<script setup>
    ...
    const { data, refresh  } = await useFetch(() => `/api/news/date/${date}`,{key:`list-${page.value}`}) ;

</script>
<template class="flex gap-1">
    
    <div class="flex flex-col p-1 md:p-0">
        <h1 class="text-2xl text-gray-700 font-semibold p-3">All News</h1>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2"  data-aos="fade-up">
