            ...
            <div v-for="info in data.data" :key="info.slug">
                <div class="flex flex-col border border-blue-100 rounded-lg ">
                    <NuxtLink :to="`/news/${info.slug}`">
                        <img :src="info.main_image" loading="" alt="News card's main image" class="rounded-md h-fit w-full hover:rotate-[0.5deg] hover:shadow-md"/>
                        <div class="p-1">
...
<script>
 const loadNews = () => {
    alert("Hi")
    let max = (window.screenY) + (window.height);
    let range = window.height - 2000;
