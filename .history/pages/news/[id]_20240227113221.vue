...
<template>
    <div class="max-h-screen">
        ...
        <swiper 
            ...
            <swiper-slide>
                <div class="flex flex-col justify-around items-center h-screen w-full bg-purple-900/40">
                    <div class="flex flex-grow flex-col justify-around items-center gap-10">
                        <div class="text-center font-semibold text-3xl text-slate-800/50">{{ data.data.news.date }}</div>
                        <div class="flex gap-10 justify-center">
                            <div class="flex items-center gap-1">
                                <Icon name="uil:eye" color="#525252" /> 
                                {{ data.data.news.total_views }} {{ data.data.randomNews.title }}
                            </div>
                            <div class="flex items-center gap-1">
                                <Icon name="uil:share" color="#525252" />  
                                {{ data.data.news.total_shares }}
                            </div>
                        </div>
                        <NuxtLink :to="`/categories/${data.data.news.category_slug}`">
                            <div class="max-w-max text-md text-blue-800 border border-blue-800 rounded-lg px-3">{{ data.data.news.category_en }}</div>
                        </NuxtLink>
                    </div>
                    <NuxtLink :to="`/news/${data.data.randomNews.slug}`" class="w-full p-2">
                        <div class="bg-zinc-700/60 rounded-lg flex justify-between items-center p-2 line-clamp-1 truncate">
                            <div class="flex flex-grow">{{ data.data.randomNews.title }}  {{ data.data.randomNews.title }}  {{ data.data.randomNews.title }}  {{ data.data.randomNews.title }}</div>
