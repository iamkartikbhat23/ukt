...
<template>
    <div class="w-full">
        <div>
            <h1 class="text-2xl font-semibold py-5">Popular News</h1>
        ...
        <template v-for="category in data.data.categories" class="flex flex-col gap-3 w-full">
            
            <div class="flex justify-between">
                <h2 class="w-10/12 text-2xl font-semibold py-5">{{ category.eng_name }}</h2>
                <div class="w-2/12 text-sm md:text-md text-blue-700 font-semibold flex gap-1">
                    View all
                    <Icon name="mdi:chevron-double-right" class="text-2xl" color="#0ea5e9" id="icnImg" />
                </div>
            </div>
            <Swiper :modules="[SwiperNavigation]"
                    :slidesPerView="1"
